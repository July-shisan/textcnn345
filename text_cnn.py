    ...
    def __init__(
      ...
      embedding_size, filter_sizes, num_filters, l2_reg_lambda=0.0):
        ...
        with tf.name_scope("output"):
            ...
            self.scores = tf.nn.sigmoid(self.logits, name="scores")
            # self.predictions = tf.argmax(self.scores, 1, name="predictions")
            self.predictions = tf.round(self.scores, name="predictions")
            # self.predictions = tf.argmax(np.array(self.scores))
        ...
        #     self.accuracy = tf.reduce_mean(tf.cast(correct_predictions, "float"), name="accuracy")
            # self.accuracy = tf.metrics.precision(self.input_y, self.predictions, name="precision-micro")[1]
